...
void updateHaptics(void)
{
    ...
    while(simulationRunning)
    {
        ...
        while (i < numHapticDevices)
        {
			...
			x << std::setprecision(2) << newForce.x;
			y << std::setprecision(2) << newForce.y;
			z << std::setprecision(2) << newForce.z;

			const std::string& x_tmp = x.str();
			const std::string& y_tmp = y.str();
			const std::string& z_tmp = z.str();

			char to_send[50];
			strcpy(to_send, "x");
			strcat(to_send, x_tmp.c_str());
			strcat(to_send, "y");
			strcat(to_send, y_tmp.c_str());
			strcat(to_send, "z");
			strcat(to_send, z_tmp.c_str());
			strcat(to_send, ";");
			...
			char serialData[50] = "i2e";
			if (sp->ReadData(serialData, 4) > 0)
			{
			//	std::string test(to_receive);
			...
			while (serialData[index] != NULL)
			{
				if (serialData[index] == 'i')
				{
					i_ind  = index;
				} else if (serialData[index] == 'e')
				{
					e_ind = index;
					break;
				}
			...
			{
				//Check whether or not indices are correctly
				change = sp->convert_to_int(serialData, i_ind, e_ind);
			}
