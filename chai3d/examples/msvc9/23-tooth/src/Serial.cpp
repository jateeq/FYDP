...
int Serial::convert_to_int(char *data, int ind_start, int ind_end)
{
  int number = 0;
  //Parse through the array to find numbers
  for (int i = ind_start+1; i < ind_end; i++)
  {
    number += (data[i]-'0')*((int) pow(10.0,(ind_end-i-1)));
  }
  return number;
}
